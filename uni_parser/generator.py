...
"""]
    ...
    if template_tag is None:
        if source_file:
            parser.append(f"\n{indent}code_lexer = Lexer(Scanner(SourceFile(source_file=os.path.join(this_path, 'sources', '{source_file}')), template_tag={template_tag}))")
        else:
            parser.append(f"\n{indent}code_lexer = Lexer(Scanner(SourceFile(source_code={source_code}), template_tag={template_tag}))")
    else:
        if source_file:
            parser.append(f"\n{indent}code_lexer = Lexer(Scanner(SourceFile(source_file=os.path.join(this_path, 'sources', '{source_file}')), template_tag='{template_tag}'))")
        else:
    ...
    if return_ast or save_ast:
        parser.append(f"\n{indent}result = EBNF.match(tracker, code_lexer, grammar='{grammar}', return_ast=True, debug=debug)")
        if save_ast:
        ...
        if return_ast:
            parser.append(f"\n{indent}return result.format()\n")
    else:
        parser.append(f"\n{indent}EBNF.match(tracker, code_lexer, grammar='{grammar}', debug=debug)")
