...
import re
from yeti_engine.main import XPLAN, SyntaxError
...
class TempliteTest(TestCase):
    ...
    def try_render(self, text, ctx=None, result=None):
        ...
        """
        actual = XPLAN(text).render(ctx or {})
        if result:
            self.assertEqual(actual, result)

    def assertSynErr(self, msg):
        pat = "^" + re.escape(msg) + "$"
        return self.assertRaisesRegexp(SyntaxError, pat)

    def test_passthrough(self):
        # Strings without variables are passed through unchanged.
        self.assertEqual(XPLAN("Hello").render(), "Hello")
        self.assertEqual(
            XPLAN("Hello, 20% fun time!").render(),
            "Hello, 20% fun time!"
            )
    ...
    def test_reusability(self):
        ...
        template = XPLAN("This is {{name|upper}}{{punct}}", globs)
