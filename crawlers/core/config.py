...
"Vary", "Via",
    "Warning", "WWW-Authenticate", "Origin", "Upgrade-Insecure-Requests",
"X_FORWARDED_FOR"
}
...
def make_random_useragent(ua_type: str = "pc"):
    """
    make a random user_agent based
    """
    ua_type = ua_type.lower()
    assert ua_type in ("pc", "phone",
                       "all"), "make_random_useragent: parameter ua_type[%s] is invalid" % ua_type
...
def get_url_legal(url: str, base_url: str, encoding=None,
                  remove_fragment=True):
    """
    create full url based on base_url + url
    """
    url_join = urllib.parse.urljoin(base_url, url, allow_fragments=True)
    url_legal = urllib.parse.quote(url_join, safe="%/:=&?~#+!$,;'@()*[]|",
                                   encoding=encoding)
    if remove_fragment:
        url_frags = urllib.parse.urlparse(url_legal, allow_fragments=True)
        url_legal = urllib.parse.urlunparse((url_frags.scheme,
                                             url_frags.netloc,
                                             url_frags.path,
                                             url_frags.params,
                                             url_frags.query, "")
