...
class FetcherThread(BaseThread):
    ...
    def running(self):
        ...
        if fetch_result == 1:
            ...
            self._thread_pool.add_task(FLAGS.PARSE, (priority, url, data, deep, content))

        # fetch failed, put back to task_queue_f and repeat later
        elif fetch_result == 0:
            self._thread_pool.add_task(FLAGS.FETCH, (priority + 1, url, data, deep, repeat + 1))
...
class ParserThread(BaseThread):
    ...
    def running(self):
        ...
        if parse_result > 0:
            ...
            #
            # if data in task_queue_p has a positive 'save' value
            if (data and data.get('save')) or (not data and url):
                try:
                    ...
                    pass
                self._thread_pool.add_task(FLAGS.SAVE, (url, data, stamp))
